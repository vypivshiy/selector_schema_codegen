...
def format_error(
    ...
    *,
    ...
    display_start, display_end = _visible_line_window(
        error.line, len(lines), context_lines
    )
    ...
    if error.path:
        parts.append(
            f"{pad} {_style('=', color=colors.dim, use_color=use_color)} {_style('scope:', color=colors.cyan, bold=True, use_color=use_color)} {error.path}"
        )
    if error.hint:
        parts.append(
            f"{pad} {_style('=', color=colors.dim, use_color=use_color)} {_style('help:', color=colors.cyan, bold=True, use_color=use_color)} {error.hint}"
        )
