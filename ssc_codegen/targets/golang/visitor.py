...
- Package name from ``ctx.meta["package"]`` (default: ``main``).
    ...
    try:
        ...
        if proc.returncode != 0:
            ...
            raise BuildTimeError(
                f"gofmt failed: {detail or 'invalid Go source'}"
            )
        return proc.stdout.decode("utf-8") if proc.stdout else source
    except subprocess.TimeoutExpired as exc:
        raise BuildTimeError("gofmt timed out after 10 seconds") from exc
    except OSError as exc:
        raise BuildTimeError(f"failed to execute gofmt: {exc}") from exc


_GO_PACKAGE_RE = re.compile(r"^[A-Za-z_][A-Za-z0-9_]*$")
_GO_KEYWORDS = {
    "break",
    "default",
    "func",
...
def validate_go_package_name(value: object) -> str:
    package = str(value or "main")
    if not _GO_PACKAGE_RE.fullmatch(package) or package in _GO_KEYWORDS:
        raise BuildTimeError(f"invalid Go package name: {package!r}")
    return package
...
class GoVisitor(BaseWalker):
    ...
    def visit_module(self, node: Module, ctx: WalkContext) -> list[str]:
        package = validate_go_package_name(ctx.meta.get("package"))
        lines: list[str] = [
            "// Code generated by ssc-gen. DO NOT EDIT.",
            "",
            f"package {package}",
    ...
    def visit_method_rest(
        ...
        if spec.body_kind == "json":
            self._require("stdJSONBody")
        if spec.body_kind == "form" and isinstance(spec.payload, dict):
            self._builder.require_import('"net/url"')
        if spec.params:
            self._builder.require_import('"net/url"')
        joined_templates = list(spec.params.values())
        if spec.body_kind == "form" and isinstance(spec.payload, dict):
            joined_templates.extend(spec.payload.values())
        if any(
            (ph := tmpl.single_placeholder()) is not None
            and ph.is_array
            and ph.style in ("csv", "pipe", "space")
            for tmpl in joined_templates
        ):
            self._builder.require_import('"strings"')
        self._builder.require_import('"' + self._http.import_path + '"')
        struct = node.parent
