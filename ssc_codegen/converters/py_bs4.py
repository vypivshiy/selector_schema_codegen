...
from ssc_codegen.converters.utils import wrap_regex_pattern

from ssc_codegen.objects import TokenType, Node, VariableState

converter = CodeConverter(templates_path="ssc_codegen.templates.python.bs4")
...
def op_css(node: Node) -> str:
    return (
        ...
        + " = "
        + VAR_R(node)
        + f".select_one({node.expression.arguments[0]!r})"
    )
...
def op_css_all(node: Node) -> str:
    return (
        ...
        + " = "
        + VAR_R(node)
        + f".select({node.expression.arguments[0]!r})"
    )
...
def op_attr(node: Node) -> str:
    ...
    if node.var_state == VariableState.DOCUMENT:
        # bs4 __getattr__ method returns list
        # need standard variable type, because most http parsers returns string
        return (
            VAR_L(node)
            + " = "
            + '" ".join('
            ...
            + f"[{attr!r}]"
            + ")"
            + f"if isinstance({VAR_R(node)}[{attr!r}], list) "
            + f"else {VAR_R(node)}[{attr!r}]"
        )
    return (
        VAR_L(node)
        + " = "
        + f'[" ".join(el[{attr}!r]) if isinstance(el[{attr}!r], list) '
        + f"else el[{attr}!r] "
        + f"for el in {VAR_R(node)}]"
    )
...
def op_text(node: Node):
    if node.var_state is VariableState.DOCUMENT:
        return VAR_L(node) + " = " + VAR_R(node) + ".text"
    return VAR_L(node) + " = " + f"[el.text for el in {VAR_R(node)}]"


@converter(TokenType.OP_ATTR_RAW)
def op_attr_raw(node: Node) -> str:
    if node.var_state is VariableState.DOCUMENT:
        return VAR_L(node) + " = " + VAR_R(node) + ".__str__()"
    return (
        VAR_L(node)
        + " = "
        + VAR_R(node)
        + f"[el.__str__() for el in {VAR_R(node)}]"
    )
...
def op_regex(node: Node) -> str:
    ...
    pattern = wrap_regex_pattern(pattern)
    return VAR_L(node) + " = " + f"re.search({pattern}, {VAR_R(node)})" + "[1]"
...
def op_regex_all(node: Node) -> str:
    pattern = node.expression.arguments[0]
    pattern = wrap_regex_pattern(pattern)
    return VAR_L(node) + " = " + f"re.findall({pattern}, {VAR_R(node)})"
...
def op_regex_sub(node: Node) -> str:
    ...
    pattern = wrap_regex_pattern(pattern)
    return f"{VAR_L(node)} = " + f"re.sub({pattern}, {repl!r}, {VAR_R(node)})"
...
def op_string_trim(node: Node) -> str:
    ...
    if node.var_state == VariableState.STRING:
        return VAR_L(node) + " = " + f"{VAR_R(node)}.strip({substr!r})"
    return VAR_L(node) + " = " + f"[s.strip({substr!r}) for s in {VAR_R(node)}]"
...
def op_string_l_trim(node: Node) -> str:
    substr = node.expression.arguments[0]
    if node.var_state == VariableState.STRING:
        return VAR_L(node) + " = " + f"{VAR_R(node)}.lstrip({substr!r})"
    return (
        VAR_L(node) + " = " + f"[s.lstrip({substr!r}) for s in {VAR_R(node)}]"
    )
...
def op_string_r_trim(node: Node) -> str:
    substr = node.expression.arguments[0]
    if node.var_state == VariableState.STRING:
        return VAR_L(node) + " = " + f"{VAR_R(node)}.rstrip({substr!r})"
    return (
        VAR_L(node) + " = " + f"[s.lstrip({substr!r}) for s in {VAR_R(node)}]"
    )
...
def op_string_replace(node: Node) -> str:
    ...
    if node.var_state == VariableState.STRING:
        return VAR_L(node) + " = " + f"{VAR_R(node)}.replace({old!r}, {new!r})"
    return (
        VAR_L(node)
        + " = "
...
def op_string_format(node: Node) -> str:
    ...
    if node.var_state == VariableState.STRING:
        return VAR_L(node) + " = " + repr(fmt_str) + f".format({VAR_R(node)})"
    return (
        VAR_L(node) + " = " + f"[{fmt_str!r}.format(s) for s in {VAR_R(node)}]"
    )
...
def op_default_end(node: Node) -> str:
    ...
    if value != None:
        value = repr(value)
    return "except Exception as e:" + f" return {value}"
