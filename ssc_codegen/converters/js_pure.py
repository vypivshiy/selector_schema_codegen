...
@JS_CONVERTER(a.Module)
def pre_module(node: a.Module, _: ConverterContext):
    lines: list[str] = []
    if node.doc:
        lines.extend(_js_docblock(node.doc.splitlines()))
    lines.extend(['"use strict";', "// autogenerated by ssc-gen. DO NOT EDIT"])
    return lines
...
def pre_struct_docstring(node: a.StructDocstring, ctx: ConverterContext):
    # Deprecated: struct docstring is emitted via _js_struct_header from
    # StructBase.doc. Kept as no-op for backward-compat handler registration.
    return None
