...
def op_attr(node: Node) -> str:
    ...
    if node.var_state == VariableState.DOCUMENT:
        return VAR_L(node) + " = " + VAR_R(node) + f".attributes[{attr!r}]"
    return (
        VAR_L(node)
        + " = "
...
def op_attr_raw(node: Node) -> str:
    if node.var_state is VariableState.DOCUMENT:
        return VAR_L(node) + " = " + VAR_R(node) + ".html"
    return (
        VAR_L(node) + " = " + VAR_R(node) + f"[el.html for el in {VAR_R(node)}]"
    )
...
def op_assert_css(node: Node) -> str:
    ...
    return "assert" + " " + VAR_R(node) + f".css_first({query!r}).html"
