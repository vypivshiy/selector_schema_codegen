...
from typing import Annotated as A, Optional  # type: ignore
from typing import List

import typer

from ssc_codegen.converters.generator import CodeGenerator
from ssc_codegen.schema import BaseSchema, FlattenListSchema, ItemSchema, DictSchema, ListSchema
...
def main(configs: A[list[str], typer.Argument(help='config files')],
         converter: A[Converters, typer.Option('-c', '--converter',
             help='code converter', show_default=False)
         ],
         output_folder: A[Path, typer.Option('-o', '--output',
         ...
         no_format: A[bool, typer.Option('--skip-format', help='skip format code')] = False,
         version: A[Optional[bool], typer.Option('--version',
             help='print version and exit', callback=_version_cb)] = None
         ):
    ...
    converter_module = importlib.import_module(f"ssc_codegen.converters.{converter}")
