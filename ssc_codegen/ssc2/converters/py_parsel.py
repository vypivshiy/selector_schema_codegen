...
def tt_init(_) -> str:
    return "def __init__(self, document: Union[str, SelectorList, Selector]):"


@converter.post(TokenType.STRUCT_INIT)
def tt_init(_):
    return "self._doc=Selector(document) if isinstance(document, str) else document"
...
def tt_imports(_: ModuleImports) -> str:
    ...
    buildin_imports += "from parsel import Selector, SelectorList\n"
