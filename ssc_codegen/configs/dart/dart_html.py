...
class Translator(ABCExpressionTranslator):
    ...
    def op_wrap_code_with_default_value(self, node: "Node", code: str, default_value: str) -> str:
    ...
    def op_string_replace(self, node: "Node", old: str, new: str, count=None) -> str:
    ...
    def op_regex_sub(self, node: "Node", pattern: str, repl: str, count=None) -> str:
        if count is None or count in (-1, "-1"):
            return ""
        reg_var = f"regex_{node.id}"
        return (f"RegExp {reg_var} = RegExp(r{pattern});\n"
    ...
    def op_assert_re_match(self, node: "Node", pattern: str) -> str:
        ...
        return (f"RegExp {re_var} = RegExp(r{pattern});\n"
